...
// Attempt to load project root .env (two levels up)
dotenv.config({ path: path.resolve(__dirname, "../../.env") });
// Fallback: local .env in this folder if still missing
if (!process.env.OPENAI_API_KEY) {
  dotenv.config({ path: path.resolve(__dirname, ".env") });
}
if (!process.env.OPENAI_API_KEY) {
  throw new Error(
    "OPENAI_API_KEY not found. Create a .env at project root with:\nOPENAI_API_KEY=your_real_key"
  );
}
