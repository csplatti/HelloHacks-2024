...
essay_feedback_prompt = PromptTemplate(input_variables=["essay", "criteria"], template=essay_feedback_prompt)

def get_prompt(essay, criteria):
   return essay_feedback_prompt.format(essay = essay, criteria = criteria)

def query_feedback(prompt):
...
response = query_feedback(get_prompt(student_essay, "professional tone"))
